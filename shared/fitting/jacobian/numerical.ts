...
export interface NumericalJacobianOptions {
   ...
   *
   * 跨尺度参数模型推荐提供。
   * 例如 { I_max: 1e4, K: 1e-6 } 让不同参数使用不同绝对步长。
   */
...
export class NumericalJacobian implements JacobianProvider {
  ...
  compute(
    ...
    if (relativeStep <= 0 || !Number.isFinite(relativeStep)) {
      throw new Error(
        `relativeStep 必须为正有限数，当前为 ${relativeStep}`,
      )
    }
    ...
    for (let j = 0; j < p; j++) {
      ...
      if (!paramName) {
        throw new Error(`paramNames[${j}] 不能为空字符串`)
      }
      ...
      if (yPlus.length !== n) {
        throw new Error(
          `fn(p + ${paramName}) 返回长度 ${yPlus.length} ≠ 预期 ${n}`,
        )
      }
      ...
      if (yMinus.length !== n) {
        throw new Error(
          `fn(p - ${paramName}) 返回长度 ${yMinus.length} ≠ 预期 ${n}`,
        )
      }

      // 还原参数对象，避免影响下一轮（虽然会被覆盖，但保险）
      paramsTrial[paramName] = paramValue

      // 填第 j 列：(yPlus[i] - yMinus[i]) / (2h)
      const denom = 2 * h
      for (let i = 0; i < n; i++) {
        jacobian[i]![j] = (yPlus[i]! - yMinus[i]!) / denom
      }
    }
