...
import type { DataArray, ParamValues } from "@shared/fitting/index.ts"
...
export type FitEquationResult =
  | ({ algorithm: "lm"; effectiveX: DataArray; effectiveY: number[]; indices: number[]; excluded: { index: number; x: number; y: number; reason: string }[] } & LevenbergMarquardtResult)
  | ({ algorithm: "odr"; effectiveX: DataArray; effectiveY: number[]; indices: number[]; excluded: { index: number; x: number; y: number; reason: string }[] } & ODRResult)
 ...
 *
 * @param equation - 公式
 * @param xData - x 数据（原始；行主序设计矩阵：rows = 样本数，cols = 自变量分量数，
 *                单变量业务可用 fitting/pre/data-shape 的 singleXToMatrix 包装）
 * @param yData - y 数据（原始）
