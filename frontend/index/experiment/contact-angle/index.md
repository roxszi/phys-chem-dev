...
> 
> 1.  [**设备垂直校准**](vertical-calibration/)。可满足接触角测量前，手机需进行的垂直校准操作需求。
> 
> 2.  [**液滴照片处理**](drop-pic-process/)。可满足接触角测量后，需对图片进行的各类处理需求，可获得最终的接触角数据。
...
[**设备垂直校准功能链接**](vertical-calibration/)
...
两种算法中，Canny算法有2个参数（因此在图片下方提供了2个滑轨），而阈值化方法有1个参数（因此在图片下方提供了1个滑轨）。为更方便的对参数进行微调，本软件提供了“切换细调/切换粗调”按钮，切换后可细化/粗化滑轨的范围值，滑轨、及细调/粗调切换按钮如下图。
...
本应用提供了3种遮罩方法，**两边遮罩**、**基线遮罩**和**中心遮罩**。遮罩切换开关、遮罩功能介绍均位于图片上方。具体如下图所示：
...
同样地，可使用“**基线遮罩**”功能手动过滤误识别的基线轮廓线及基线下方的各类误识别轮廓线。**此步骤不用追求极致精确，因本软件的迭代过滤算法可实现遮罩线附近的误识别轮廓被自动过滤**。

通过合理使用参数调节滑轨与遮罩功能，使真实轮廓线尽可能多、误识别轮廓线尽可能少。即可实现较好的轮廓识别效果。

> [!info] Tips：
> 
> 轮廓线的数量一般而言完全满足拟合需求。因此尽可能调节滑轨，使液滴中部误识别轮廓线尽可能少。
> 
...
除弹窗通知外，文件名、接触角平均值、相对偏差值将以表格形式展示于页面下部：
...
本业务在接触角图片处理软件的编写过程中借鉴了ImageJ中接触角插件的部分思路。
