...
<template><MySpace>
  ...
  <t-slider
    ...
    :marks="[0, 85, 170, 255]"
    :min="0" :max="255" :step="1" v-model="thresholdNum"
  /><br />
...
async function onPicChange(event) { try {
  ...
  imgElement.remove()
  // 如果全局灰度图Mat对象存在，则先删除
  if (contactAngleObj.matGray) { contactAngleObj.matGray.delete() }
  // 初始化全局灰度图Mat对象
...
function chooseContour(thresholdNumber) { try {
  ...
  // 定义最小外接圆的颜色为红色（其实在8位灰度图像中，颜色值都是灰度值，也就是白色）
  const contoursColor = new contactAngleObj.cv.Scalar(255, 0, 0)
  // 轮廓粗细
